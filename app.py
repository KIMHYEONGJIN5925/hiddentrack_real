...
def save_comment():
    ...
    return jsonify({'msg': '코멘트 저장완료!'})


@app.route('/detail', methods=['GET'])
def view_comments():
    comments = list(db.comment.find({}, {'_id': False}))
    return jsonify({'all_comments': comments,'message':'plzplz'})

@app.route('/detail', methods=['GET','POST'])
def board_detail():
    detail_receive = request.form['detail_give']
    infos = list(db.cafe.find({'name':detail_receive}, {'_id': False}))
    real_info = infos[0]['name']
    return jsonify({'cafe_infos': real_info}), render_template('detail.html')
